<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="账单编号" prop="zhangdanbianhao">
            <el-input v-model="ruleForm.zhangdanbianhao" 
                placeholder="账单编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.zhangdanbianhao" label="账单编号" prop="zhangdanbianhao">
              <el-input v-model="ruleForm.zhangdanbianhao" 
                placeholder="账单编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="消费金额" prop="xiaofeijine">
          <el-input v-model="ruleForm.xiaofeijine" 
              placeholder="消费金额" clearable  :readonly="ro.xiaofeijine"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  created() {
	this.ruleForm.xiaofeiriqi = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='zhangdanbianhao'){
            this.ruleForm.zhangdanbianhao = obj[o];
	    this.ro.zhangdanbianhao = true;
            continue;
          }
          if(o=='mingcheng'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiaofeizhangdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiaofeizhangdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
